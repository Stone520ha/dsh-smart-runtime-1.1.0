...
export function evaluateLivenessPreStep(state, config) {
    if (!config.livenessGuard)
...
export function observeStopBoundary(state) {
...
export function maySteerAtStop(state, config) {
    return config.livenessGuard && state.liveness.stopSteers < config.maxStopSteers;
}
export function recordStopSteer(state) {
    state.liveness.stopSteers += 1;
}
export function shouldAllowDegradedConvergence(state, config) {
    if (!config.livenessGuard)
        return false;
...
export function livenessRecoveryPrompt(state, assessment, reason) {
...
export function repeatedToolPrompt(state, reason) {
