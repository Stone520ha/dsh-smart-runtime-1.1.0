...
import type { Config, RuntimeState, SessionLike } from './types.js'
...
const SOURCE: MessageSource = { kind: 'plugin', plugin: 'smart-runtime' }

export function applySmartRuntime(ctx: Context, rawConfig: Config): void {
  ...
  ctx.on('agent/pre-step', async ({ agent, messages, turn, step }, next): Promise<PreStepDecision> => {
    ...
    if (!state) {
      const routed = config.strategyRouting ? routeStrategy(prompt) : { strategy: 'direct' as const, confidence: 1, scores: [] }
    ...
    if (liveness.kind === 'hard') {
      ...
      if (config.telemetry) ctx.logger.warn(`[smart-runtime] hard liveness stop turn=${turn} step=${step}: ${liveness.reason}`)
      return { kind: 'reject' }
    }
    ...
    if (budget.kind === 'hard') {
      ...
      repository.saveSnapshot(session, state)
      if (config.telemetry) logBudgetStop(ctx.logger, state, budget.reasons)
      return { kind: 'reject' }
    }
    ...
    if (step === 1 && config.strategyRouting) {
      additions.push(notice(
        `Execution mode: ${state.strategy}.\n${strategyInstruction(state.strategy)}\nTreat this as orchestration guidance; do not narrate hidden reasoning.`,
    ...
    repository.saveSnapshot(session, state)
    if (config.telemetry) logTurnState(ctx.logger, state)
  ...
  ctx.on('tools/post-execute', async (exec: ToolExecution, result, next): Promise<PostToolDecision> => {
    ...
    if (turn === undefined) return downstream
  ...
  ctx.on('agent/turn-stopping', ({ agent, turn }) => {
    ...
    if (shouldAllowDegradedConvergence(state, config)) {
      repository.addCheckpoint(session, state, checkpoint(
        state,
        'liveness',
        state.budget.steps,
        `bounded convergence after stopSteers=${state.liveness.stopSteers}, sameStateVisits=${state.liveness.sameEvidenceStopVisits}; score=${assessment.score.toFixed(2)}`,
      ))
      repository.saveSnapshot(session, state)
      return
    }
    ...
    if (config.telemetry) logVerification(ctx.logger, state, assessment)
    if (assessment.pass) {
    ...
    repository.addCheckpoint(session, state, checkpoint(
      state,
      recoveringFromSameState ? 'liveness' : 'verification',
      state.budget.steps,
      `steer=${state.liveness.stopSteers} round=${state.verificationRounds} score=${assessment.score.toFixed(2)} blockers=${assessment.blockers.join(' | ')}`,
    ))
    repository.saveSnapshot(session, state)
  ...
  ctx.on('agent/disposed', ({ agent }) => {
    ...
    repository.dispose(agent.session as unknown as SessionLike)
  })
}
...
function checkpoint(
  state: RuntimeState,
  kind: RuntimeState['checkpoints'][number]['kind'],
  step: number,
  note: string,
): RuntimeState['checkpoints'][number] {
  const at = Date.now()
  return {
...
function effectiveToolResult(
  result: Readonly<ToolExecutionResult>,
  decision: PostToolDecision,
): { isError: boolean; content?: unknown; error?: { message?: string }; value?: unknown } {
