...
export function reflectionTrigger(state: RuntimeState, config: ResolvedConfig): ReflectionTrigger {
  ...
  if (noProgressSteps >= config.maxNoProgressSteps) reasons.push(`no observable progress for ${noProgressSteps} steps`)
  return { triggered: reasons.length > 0, reasons }
}

export function prepareReflection(state: RuntimeState): void {
...
export function reflectionPrompt(state: RuntimeState, reasons: readonly string[]): string {
