...
export function createPlan(strategy: Strategy): PlanItem[] {
...
export function updatePlan(plan: readonly PlanItem[], observation: ToolObservation): PlanItem[] {
  ...
  if (candidateIndexes.length === 0) return next

  const index = candidateIndexes.find(i => next[i]!.status === 'active' || next[i]!.status === 'pending') ?? candidateIndexes[0]!
  const item = next[index]!
  if (!item.evidenceIds.includes(observation.id)) item.evidenceIds.push(observation.id)
...
export function markPlanForReflection(plan: readonly PlanItem[]): PlanItem[] {
  const next = plan.map(item => ({ ...item, evidenceIds: [...item.evidenceIds] }))
...
export function planCompletion(plan: readonly PlanItem[]): number {
  if (plan.length === 0) return 1
  const done = plan.filter(item => item.status === 'done' || item.status === 'skipped').length
  return done / plan.length
}

function activateNext(plan: PlanItem[]): void {
  if (plan.some(item => item.status === 'active')) return
  const next = plan.find(item => item.status === 'pending')
  if (next) next.status = 'active'
}

function candidatesForObservation(plan: readonly PlanItem[], observation: ToolObservation): number[] {
  ...
  for (let i = 0; i < plan.length; i++) {
    ...
    if (isLikelyResearchTool(tool) && /research|evidence|source|调研|证据|资料/.test(title)) indexes.push(i)
    if (isLikelyFileTool(tool) && /inspect|repository|input|约束|代码|环境|read/.test(title)) indexes.push(i)
    if (isLikelyExecutionTool(tool) && /implement|execute|build|修改|实现|执行|produce/.test(title)) indexes.push(i)
    if (isLikelyTestTool(tool, text) && /verify|test|check|验证|测试/.test(title)) indexes.push(i)
  }
  return indexes
}

function planRows(strategy: Strategy): Array<[string, string]> {
  switch (strategy) {
    case 'code':
      return [
