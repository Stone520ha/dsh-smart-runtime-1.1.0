...
export function shouldInjectContext(state: RuntimeState, step: number, config: ResolvedConfig): boolean {
  if (!config.contextInjection) return false
...
export function buildRuntimeContext(
  ...
  delegation?: DelegationAdvice,
): string {
  if (!config.contextLayering) return buildFlatContext(state, config, delegation)
...
function stableGoalLayer(state: RuntimeState): string {
  ...
  if (state.goal.acceptanceCriteria.length > 0) {
    ...
    for (const item of state.goal.acceptanceCriteria.slice(0, 8)) lines.push(`- ${preview(item, 260)}`)
  }
  if (state.goal.constraints.length > 0) {
    lines.push('Constraints:')
    for (const item of state.goal.constraints.slice(0, 6)) lines.push(`- ${preview(item, 240)}`)
  }
  return lines.join('\n')
}

function activePlanLayer(state: RuntimeState): string {
  ...
  if (active) lines.push(`Next concrete milestone: ${active.title} — done when: ${preview(active.doneWhen, 260)}`)
  return lines.join('\n')
}

function workingEvidenceLayer(state: RuntimeState, config: ResolvedConfig): string {
...
function distilledStateLayer(state: RuntimeState): string {
  ...
  if (blocked.length > 0) {
    ...
    for (const item of blocked.slice(0, 4)) lines.push(`- ${item.title}${item.note ? `: ${preview(item.note, 180)}` : ''}`)
  }
  const latestFailure = [...state.observations].reverse().find((item: ToolObservation) => item.isError)
  if (latestFailure) lines.push(`Latest failure: ${latestFailure.tool}: ${preview(latestFailure.resultPreview, 260)}`)
  return lines.join('\n')
}

function archiveControlLayer(
  state: RuntimeState,
  config: ResolvedConfig,
  delegation?: DelegationAdvice,
): string {
  ...
  if (recentCheckpoints.length > 0) {
    ...
    for (const cp of recentCheckpoints) lines.push(`- ${cp.kind}@${cp.step}: ${preview(cp.note, 180)}`)
  }
  ...
  if (delegation?.shouldConsiderDelegation) {
    ...
    if (delegation.preferredProviders.length > 0) lines.push(`Preferred for this strategy: ${delegation.preferredProviders.join(', ')}`)
    if (delegation.isolationGuidance) lines.push(`Isolation rule: ${delegation.isolationGuidance}`)
  }
...
function buildFlatContext(
  ...
  if (state.plan.length > 0) lines.push(`Plan progress: ${Math.round(planCompletion(state.plan) * 100)}%`)
  for (const obs of state.observations.slice(-config.maxEvidenceItems)) {
    lines.push(`- ${obs.isError ? 'FAIL' : 'OK'} ${obs.tool}: ${preview(obs.resultPreview || obs.summary, config.evidencePreviewChars)}`)
  }
  lines.push(`Budget: steps=${state.budget.steps}/${config.hardMaxStepsPerTurn}, tools=${state.budget.toolCalls}/${config.maxToolCallsPerTurn}, errors=${state.budget.errors}/${config.maxErrorsPerTurn}`)
  if (delegation?.shouldConsiderDelegation) lines.push(`Delegation: ${delegation.reason}`)
  return capContext(lines.join('\n'), config.contextMaxChars)
}

export function summarizeEvidence(observations: readonly ToolObservation[]): string[] {
