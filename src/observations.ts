...
export function applyObservationToBudget(state: RuntimeState, observation: ToolObservation): void {
  const budget = state.budget
  budget.toolCalls += 1

  if (observation.isError) budget.consecutiveToolFailures += 1
  else budget.consecutiveToolFailures = 0
  ...
  budget.maxRepeatedFingerprintRun = Math.max(
    budget.maxRepeatedFingerprintRun,
    budget.currentRepeatedFingerprintRun,
  )
  ...
  budget.maxRepeatedOutcomeRun = Math.max(
    budget.maxRepeatedOutcomeRun,
    budget.currentRepeatedOutcomeRun,
  )
...
function resultTextOf(result: ToolResultLike): string {
  if (result.isError && result.error?.message) return result.error.message
  if (result.content !== undefined) return contentToText(result.content)
  if (result.value !== undefined) return safeString(result.value)
...
function contentToText(content: unknown): string {
  ...
  for (const block of content) {
    ...
    if (block && typeof block === 'object') {
      ...
      else out.push(safeString(record))
    }
  }
  return out.join('\n')
}
