import type { GoalState, Strategy } from './types.js'
...
export function createGoal(prompt: string, strategy: Strategy, confidence: number): GoalState {
...
function deriveObjective(prompt: string): string {
  ...
  if (lines.length === 0) return 'Complete the current user request.'
  const first = lines[0]!
  if (first.length <= 280) return first
  return `${first.slice(0, 277)}…`
}

function strategyCriteria(strategy: Strategy, prompt: string): string[] {
  switch (strategy) {
    case 'code':
