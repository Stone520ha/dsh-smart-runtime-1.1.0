...
export function assessVerification(state: RuntimeState, config: ResolvedConfig): VerificationAssessment {
  const observations = state.observations
  const success = observations.filter(item => !item.isError)
  const failed = observations.filter(item => item.isError)
  const checklist: string[] = [...state.goal.acceptanceCriteria]
  ...
  switch (state.strategy) {
    case 'code': {
      ...
      if (mutationRequested) {
        if (!hasMutation) blockers.push('No successful code/file mutation evidence has been observed for this requested code change.')
        else score += 0.12
        if (!hasTestEvidence) blockers.push('No convincing test, typecheck, build, compile, or lint evidence has been observed yet.')
        else score += 0.16
      } else {
        if (success.length === 0) blockers.push('No observable repository/code evidence has been inspected for this code-reading task.')
        else score += 0.12
      }

      if (!hasExecution && mutationRequested) warnings.push('No successful execution/build command has been observed.')
      else if (hasExecution) score += 0.08
      break
    }
    case 'research': {
      ...
      if (researchTools.length < 2) blockers.push('Research evidence is too thin; fewer than two successful research observations are present.')
      else score += 0.12
      if (distinctOutcomes < 2) blockers.push('Research observations do not yet contain at least two distinct evidence outcomes.')
      else score += 0.06
      if (distinctSurfaces < 2 && researchTools.length >= 2) warnings.push('Evidence came through one research surface; cross-check important claims when another source path is available.')
      else if (distinctSurfaces >= 2) score += 0.04
      break
    }
    case 'analysis':
      if (success.length === 0) warnings.push('No external/tool evidence was observed; verify that this analysis genuinely required none.')
      else score += 0.08
      break
    case 'design':
      if (success.length === 0) warnings.push('No observable artifact/tool evidence was seen; ensure the requested concrete design deliverable actually exists.')
      else score += 0.06
      break
    case 'long-task':
      requiresObservableAction = true
      if (completion < 0.6) blockers.push(`Plan completion is low (${Math.round(completion * 100)}%).`)
      if (success.length < 2) warnings.push('Long task has little observable execution evidence.')
      break
    default:
      score += 0.2
  }
...
export function verificationPrompt(state: RuntimeState, assessment: VerificationAssessment): string {
  const lines = [
  ...
  if (assessment.blockers.length > 0) {
    ...
    for (const item of assessment.blockers) lines.push(`- ${item}`)
  }
...
export function shouldRunVerification(state: RuntimeState, config: ResolvedConfig): boolean {
